...
theme.ajaxCart = (function(module, $) {
  ...
  adjustCart = function() {
    ...
    $quantityRemove.length && $quantityRemove.forEach((removeButton) => {
      removeButton.addEventListener("click", (e) => {
        ...
        if (removeButton.parentElement.parentElement.parentElement.querySelector('div.grid__item.flex-auto.bundle')){
          var request = '/cart/update.js?';
          let bundle = document.querySelectorAll('div.grid__item.flex-auto.bundle.'+ removeButton.parentElement.parentElement.parentElement.querySelector('div.grid__item.flex-auto.bundle').classList[3]);
          [].forEach.call(bundle, item=>{request = request + item.id + '=0&'});
          fetch(request, {method: 'post', credentials: 'include'}).then(cart=>{
  ...
  adjustCartCallback = function(cart) {
    ...
    setTimeout(function () {
      ...
      buildCartPage(cart);
      isUpdating = false;
      if (document.querySelector('p#fsh')){
        document.querySelector('p#fsh>span').style.width = 100 * cart.total_price / document.querySelector('p#fsh').getAttribute('fsh') + '%';
        if (document.querySelector('p#fsh').getAttribute('fsh') > cart.total_price){
          document.querySelector('p#fsh+p').innerHTML = document.querySelector('p#fsh').getAttribute('nofsh').replace('[left]', theme.Currency.formatMoney(document.querySelector('p#fsh').getAttribute('fsh') - cart.total_price, settings.moneyFormat));
        } else {
          document.querySelector('p#fsh+p').innerHTML = document.querySelector('p#fsh').getAttribute('fshtext');
        }
      }
    }, 150);
  };
